...
def build() -> AppContainer:
	...
	str_data_path = str(
		Path(YAML_INPUTS["import_trades"]["data_path"]).expanduser() / str(_cls_dates.curr_date())
	)
	...
	cls_db_url = URL.create(
		...
		port=int_port,
		database=str_dbname,
		query={"options": f"-c search_path={TAXPAYER}"},
	)
	cls_db_session = DatabaseSession(cls_db_url.render_as_string(hide_password=False))
